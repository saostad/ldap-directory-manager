 ...
 * @note result can be multiple entry BUT it just returns the first entry.
 */
...
export interface UserGetAllConfigs<User> extends GetUserInputConfigs<User> {
  /** do search on this attribute for provided criteria
   * @default "userPrincipalName" */
  criteriaAttribute?: string;
}
export type UserGetAllFnInput<User> = {
  criteria?: string;
  configs: UserGetAllConfigs<User>;
};

/** @description if criteria provided, will search against attribute provided on configs
 * - ["*"] from all users
...
export async function userGetAll<User = any>({
  ...
  /** if criteria provided filter based on criteriaAttribute */
  if (criteria) {
    qGen.where({
      field: configs.criteriaAttribute ?? "userPrincipalName",
      action: "substrings",
      criteria,
    });
  }
...
export async function userGetLockedOut<User = any>({
  criteria,
  configs,
}: UserGetAllFnInput<User>) {
  ...
  const { query } = qGen
    .whereAnd({ field: "lockoutTime", action: "greaterOrEqual", criteria: "1" })
    .whereAnd({ field: "objectClass", action: "equal", criteria: "user" })
    .whereOr({ field: "objectClass", action: "equal", criteria: "person" })
    .whereNot({
      field: "objectClass",
